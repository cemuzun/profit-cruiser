...
import { scrapeSegment, CITIES, WINDOWS, pool } from "./scraper.mjs";
...
try {
  const t0 = Date.now();
  const vehicles = await scrapeSegment(null, citySlug, win, minP, maxP, null);
  ...
  if (vehicles.length === 0) {
    console.error("\n⚠ Zero vehicles — check ZYTE_API_KEY, Zyte balance, or HTML structure.");
    process.exitCode = 2;
  }
} catch (e) {
